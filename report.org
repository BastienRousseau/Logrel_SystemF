...
- a function $is\_value: expr \rightarrow Prop$ that express which expressions are the
...
\[\mathrm{Norm}(e) \eqdef \exists v \in \mathrm{Val}.\ e \step^{*} v\]
and the parameterized version
\[\mathrm{Norm}_{P}(e) \eqdef \exists v \in \mathrm{Val}.\ e \step^{*} v \wedge P(v)\]
...
\[
\typed{\Delta';\Gamma'}{e_{1} \approx^{\mathrm{ctx}} e_{2} }{\tau'}
\eqdef
 \forall E\ :\ (\Delta;\Gamma \vdash \tau) \Rightarrow (\bullet;\bullet \vdash \unit).\
 (E[e_{1}] \Downarrow v \Leftrightarrow E[e_{2}] \Downarrow v)
\]
where $e \Downarrow v \eqdef e \mstep v$ and 
\begin{mathpar}
\inferH
{Ctx-Typing}
{\typed{\Delta;\Gamma}{e}{\tau}
\\
\typed{\Delta';\Gamma'}{E[e]}{\tau'}
}
{ E\ :\ (\Delta;\Gamma \vdash \tau) \Rightarrow (\Delta';\Gamma' \vdash \tau')}
\end{mathpar}

It states that two expressions $e_{1}$ and $e_{2}$ of type $\tau$ are contextually
equivalent if and only if
for any context that has a hole of type $\tau$, and produces a closed expression of
type $\unit$ (see \ref{Ctx-Typing}), filling the hole with $e_{1}$ or $e_{2}$
will both reduces to the same value $v$ (which actually has to be $\unitt$).
