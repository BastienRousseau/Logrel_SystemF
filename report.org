...
#+title: Formalizing logical relation for SystemF type safety in Coq
...
\begin{abstract}
Type safety is a language property that ensure that any well-typed closed
program is safe to execute. While syntactic approaches have been widely used to
prove type safety, another proof method based on logical relation has been shown
to be efficient to prove such language properties. During the lecture, we have
defined, on paper, a logical relation and used it to prove type safety of
SystemF. Because there is many details, paper proof are prone to errors.
Moreover, the encoding of some data structures, as well as their properties, are
often implicits. Proof-assistant such as Coq requires to explicit everything in
the implementation, prove every single properties, and helps to keep track of
every minute details. To fill the gap between paper proof and a proof-assistant
implementation, we propose an implementation in Coq of type safety of SystemF,
...
well-typed closed term[fn::In the document, we use "term" and "expression" interchangeably]
of the language will never reach a state in which the semantic says nothing.
...
data structures are sometimes quite implicit. Proof assistants, such as Coq,
make sure that every single properties has been proved. The interactive theorem
prover helps one to keep track of every minute details. Thus, we get more
confidence by implementing logical relation to prove type safety in Coq.
...
corresponding Coq code[fn::Some representation details might differ.].
...
representation is not well-suited for the implementation.

The semantic is a small-step semantic with a call-by-value strategy.
The notation $\subst{e}{v}{x}$ denotes the capture-avoiding substitution of the
expression variable $x$ by the value $v$. We note $\mstep$ the transitive
closure of the non-head reduction relation $\step$. The syntax and the semantic
of the languge are standard.

\input{figures/typingSF1}
Figure \ref{fig:typingSF1} shows the types and the typing rules of SystemF.
We note $\typed{\tyctx;\ \exprctx}{e}{\ty}$ the typing judgment stating that
...
The type safety theorem is the composition of two lemmas using the logical
relation:
...
The second lemma is very straightforward, because the safety is baked in the
logical relation itself.
The first lemma is the interesting part of the proof. In order to prove it, we
first need to generalize the lemma. The generalized version is called the
...
We say that /the substitution $\sfun$ satisfies the typing context $\exprctx$
for the predicate $P \in \mathrm{Type} \rightarrow \mathrm{Expr} \rightarrow \PP$/, written
$\sfun \Mapsto_{P} \exprctx$, if for all expression
...
The FTLR states that if a closed term is well-typed,
the term substituted with $\sfun$ is in the logical relation, with
$\sfun$ that satisfies the typing context $\exprctx$ for the logical relation.
...
(1), but the lack of evaluation context prevents to define the
...
more tedious. The semantic has indeed two reduction relations: in particular,
the non-head reduction relation requires us to destruct the evaluation context.
\todo{is there a better way to describe it?}
...
Named binders is a way to represent the binders, such that all the occurrences of
...
we have to make sure that the substitution is capture-avoiding.
...
replaces the free variable 0[fn::under $n$ lambda abstraction, it is represented by $n$]
by $v$, and renames all the other variable consequently.
...
the proofs, in particular for the \href{thm:logrelsubst}{substitution lemma}
...
** Substitution lemmas
One of the most usefull property of the logical relation is the /substitution
...
the proof obligation ends up to be
\[
...
:END:
In this section, we discuss about different ways in which the project could be
...
the language, on the contrary to the logical relation which is defined over the
type structure.
...
implementation, it result in the definition of a class that expresses what is a
